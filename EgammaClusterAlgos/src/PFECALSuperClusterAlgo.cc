...
namespace {
  ...
  bool isClustered(const CalibClusterPtr& x,
		   ...
		   const std::vector<double> pMust,
                   ...
                   const std::string pMustLocalFile)
  { 
    const double dphi = std::abs(TVector2::Phi_mpi_pi(seed->phi() - x->phi()));
    ...
    if(type == PFECALSuperClusterAlgo::kMustache) {
      reco::MustacheKernel::setMustacheParameters(pMust, pMustLocalFile);
...
void PFECALSuperClusterAlgo::buildSuperCluster(CalibClusterPtr& seed, CalibClusterPtrVector& clusters)
{
  ...
  auto isClusteredWithSeed = std::bind(isClustered, _1, seed,_clustype,useDynamicDPhi_, etawidthSuperCluster, phiwidthSuperCluster,pMustache_,pDynPhiWin_,pMustacheLocalFile_);
