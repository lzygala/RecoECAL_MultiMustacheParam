...
PFECALSuperClusterProducer::PFECALSuperClusterProducer(const edm::ParameterSet& iConfig)
{
  ...
  //std::vector<double> localParameters = iConfig.getParameter<std::vector<double>>("localParameters");

  //Regional Mustache Parameters
  // dynamic dPhi window                                                                                                                                                                             
  ...
  edm::FileInPath mustacheParametersLocalFileInPath = iConfig.getParameter<edm::FileInPath>("mustacheParametersLocalFileInPath");

  std::string mustacheParametersLocalFile_ = mustacheParametersLocalFileInPath.fullPath();
  ...
  superClusterAlgo_.setMustacheParameters(mustacheParams_,mustacheParametersLocalFile_);
...
void PFECALSuperClusterProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::FileInPath>("mustacheParametersLocalFileInPath",edm::FileInPath("RecoEcal/EgammaClusterProducers/data/mustacheLocalParameters_phase1.txt"));
